...
def access_token_required(handler):
    """
         Decorator for checking if there's a valid Alfresco Token
         with the current session.
         Will also try to to renew it if is expired
    """
    ...
    def check_token(self, *args, **kwargs):
        try:
            ...
            if 401 == 401:
                ...
                go_url = self.uri_for('home', **{'continue': continue_url})
...
def user_required(handler):
    ...
    def check_login(self, *args, **kwargs):
        ...
        continue_url = self.request.path_url + query_string
        login_url = self.uri_for('home', **{'continue': continue_url})
